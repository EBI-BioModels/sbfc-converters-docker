...
Tomcat listens on `127.0.0.1:8095` (localhost only — Apache handles external traffic).

### Local testing (before Apache is configured)

```bash
curl http://localhost:8095/tools/converters/
```
...
Do **not** open port 8095 — the container binds to `127.0.0.1` only.
...
- `/tools/converters` is proxied to `http://localhost:8095/tools/converters` (the Docker container). This block must appear **before** the catch-all proxy rule.
